...
export default (lang, country, mergiWords) => {
  ...
  /**
   * Return first element with given class or throw error if it does not exist.
  ...
  const getChild = (el, classname) => {
    ...
    if (!element) {
      throw new Error(
          `Bad page. No element with class="${classname}" in element`);
    }
    return element;
  };
  ...
  /** Set the CSS classes that will trigger an animation
   * to flip the card to show the answer side.
   * @return {void}
   */
  const flip = () => {
  ...
  /** Set the CSS classes that will trigger an animation to unflip the
   * card to show the question side.
   * @return {void}
   */
  const unflip = () => {
    unflipSay();
  ...
  */
   ...
   * @return {function()} a function that updates deck with user response
   */
